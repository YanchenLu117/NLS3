...
class GPTWrapper(LLMWrapper):
    ...
    def __init__(
        ...
        max_backoff=60.0,
        port=6832,
        timeout=int(os.environ.get('NLSS_LLM_TIMEOUT', '120')),
        redis_kwargs: Dict = {},
        **kwargs,
    ):
        super().__init__(
        ...
        self.api = OpenAI(base_url=os.environ.get('OPENAI_BASE_URL') or os.environ.get('NLSS_LLM_BASE_URL') or None)
    ...
    def _batched_generate(
        ...
        **kwargs,
        ...
        client = AsyncOpenAI(base_url=os.environ.get('OPENAI_BASE_URL') or os.environ.get('NLSS_LLM_BASE_URL') or None)
        ...
        for r in resp:
            input_cost = r.usage.prompt_tokens * MODEL_COSTS.get(model, {'input': 0.0, 'output': 0.0})['input'] / 1000000
            output_cost = r.usage.completion_tokens * MODEL_COSTS.get(model, {'input': 0.0, 'output': 0.0})['output'] / 1000000
        ...
        **kwargs,
        ...
        for _ in range(self.max_retry):
            try:
                ...
                input_cost = resp.usage.prompt_tokens * MODEL_COSTS.get(model, {'input': 0.0, 'output': 0.0})['input'] / 1000000
                output_cost = resp.usage.completion_tokens * MODEL_COSTS.get(model, {'input': 0.0, 'output': 0.0})['output'] / 1000000
                self.total_cost += input_cost + output_cost
